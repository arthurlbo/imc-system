...
const TableCell = ({ children, colSpan, width }: TableCellProps) => {
    return (
        <Table.Cell py={4} borderColor="secondary" colSpan={colSpan} textAlign={colSpan ? "center" : "left"} w={width}>
            {children}
        </Table.Cell>
    );
};
        ...
        <Skeleton loading={isLoading} w="100%" h="100%" mt={4}>
            <Flex direction="column" w="full" h="auto" gap={8} justify="end" align="end">
                <Flex
                    ...
                    gap={4}
                >
                    <Flex direction={{ base: "column", md: "row" }} w="full" align="center" gap={4}>
                        <EvaluatorsFilter />
                        <StudentsFilter />
                    </Flex>
